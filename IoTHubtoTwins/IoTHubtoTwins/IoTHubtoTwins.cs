...
#pragma warning restore AZF0001 // Suppress async void error
        {
            ...
            try
            {
                ...
                if (eventGridEvent != null && eventGridEvent.Data != null)
                {
                    ...
                    var Current = deviceMessage["body"]["Current"];
                    // </Find_device_ID_and_temperature>

                    log.LogInformation($"Device:{deviceId} windSpeed is:{windSpeed} windDirection is:{windDirection} Power is:{Power} Current is:{Current}");

                    // <Update_twin_with_device_temperature>
                    var updateTwinData = new JsonPatchDocument();
                    updateTwinData.AppendReplace("/windSpeed", windSpeed.Value<double>());
                    updateTwinData.AppendReplace("/windDirection", windDirection.Value<double>());
                    updateTwinData.AppendReplace("/Power", Power.Value<double>());
                    updateTwinData.AppendReplace("/Current", Current.Value<double>());
